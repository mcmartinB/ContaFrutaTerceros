...
type
  ...
  TDMDContabilizacion = class(TDataModule)
    ...
    QLineasAbono: TQuery;
    QEmpresaProcedencia: TQuery;
    QSeccionContable: TQuery;
    QDirSum: TQuery;
    QTipoIva: TQuery;
    Database: TDatabase;
    QEmailAdmi: TQuery;
  ...
  private
    ...
    procedure Tercero;
    function  FechaVencimiento( Const AFecha: TDateTime ): TDateTime;
    function  Divisa: string;
    function  CambioDivisa: string;
    function  FacturaAsociada: String;

    function  AnyadirSeccionesFactura: boolean;
    function  AnyadirSeccionesAbono: boolean;
    procedure PutSeccion( const ASeccion, AUnidad: string; const AImporte, AGastosFac, AUnidades: Real);
    procedure SeccionesSalidasFactura;
    procedure SeccionesSalidasAbono;
    procedure SeccionesUnicaAbono;
    procedure SeccionesDistribuirAbono( const ALineas: Integer; const AKilos: Real );
    procedure ObtenerGastos;

    procedure InsertarLineaTFactura( const AFileName: string );
    procedure InsertarLineaTAbono( const AFileName: string );
    procedure InsertarLineaD( const ALinea: integer );
    procedure InsertarLineaA( const ALinea: integer );
    procedure InsertarLineaEFactura;
    procedure InsertarLineaEAbono;
  ...
  public
    ...
    procedure ContabilizarAbonoAutomatico( const AFileName: string );
    procedure ContabilizarAbonoManual( const AFileName: string );
...
uses Variants, Math, bTextUtils, UAutocontabilizar;
...
begin
  ...
  begin
    ...
    SQL.Add('and fecha_factura_f > ''30/6/2006'' ');
    SQL.Add('and n_factura_f between :facturaini and :facturafin');
    SQL.Add('and  NVL( contabilizado_f, ''N'' ) = ''N''  ');
    //Factura automatica/Abonos
    SQL.Add('and  not ( tipo_factura_f = 380 and concepto_f = ''M'' ) ');
    Prepare;
  end;
  ...
  begin
    SQL.Clear;
    SQL.Add('select dir_sum_sc, n_factura_sc, fecha_factura_sc ');
  ...
  begin
    ...
    SQL.Add(' order by fecha_albaran_fal, n_albaran_fal');
    Prepare;
  end;

  with QEmpresaProcedencia do
  begin
    SQL.Clear;
    SQL.Add(' select emp_procedencia_sl, centro_origen_sl, sum(kilos_sl) kilos_sl ');
    SQL.Add(' from frf_salidas_l ');
    SQL.Add(' where empresa_sl = :empresa ');
    SQL.Add(' and n_albaran_sl = :albaran ');
    SQL.Add(' and fecha_sl = :fecha ');
  ...
  with QSeccionContable do
  begin
    SQL.Clear;
    SQL.Add(' select sec_contable_sc ');
    SQL.Add(' from frf_secc_contables ');
    SQL.Add(' where empresa_sc = :empresa ');
    SQL.Add(' and centro_sc = :centro ');
    SQL.Add(' and producto_sc = :producto ');
    Prepare;
  end;
...
begin
  ...
  if QSalidas.Prepared then
    QSalidas.UnPrepare;
  QSeccionesFactura.Close;
  if QSeccionesFactura.Prepared then
    QSeccionesFactura.UnPrepare;

  QLineasAbono.Close;
  if QLineasAbono.Prepared then
    QLineasAbono.UnPrepare;
  QEmpresaProcedencia.Close;
  if QEmpresaProcedencia.Prepared then
    QEmpresaProcedencia.UnPrepare;
  QSeccionContable.Close;
  if QSeccionContable.Prepared then
    QSeccionContable.UnPrepare;
  ...
  if QDirectorioConta.Prepared then
    QDirectorioConta.UnPrepare;
  QEmailAdmi.Close;
  if QEmailAdmi.Prepared then
    QEmailAdmi.UnPrepare;
...
function TDMDContabilizacion.FechaVencimiento( Const AFecha: TDateTime ): TDateTime;
begin
  if QDocs.FieldByName('prevision_cobro_f').Value <> NULL then
    ...
    result:= GetFechaVencimiento( AFecha, iIncDias, iDiaPago1, iDiaPago2 );
end;
...
procedure TDMDContabilizacion.SeccionesSalidasFactura;
var
  rGastos: real;
begin
  with QSeccionesFactura do
  begin
    ...
    begin
      ...
      if QSalidas.FieldByName('palets_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('palets_s').AsFloat * rGastosPalet ) / QSalidas.FieldByName('palets_sc').AsFloat );
      if QSalidas.FieldByName('kilos_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('kilos_s').AsFloat * rGastosKilo ) / QSalidas.FieldByName('kilos_sc').AsFloat );
      if QSalidas.FieldByName('cajas_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('cajas_s').AsFloat * rGastosCaja ) / QSalidas.FieldByName('cajas_sc').AsFloat );
      if QSalidas.FieldByName('neto_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('neto_s').AsFloat * rGastosImporte ) / QSalidas.FieldByName('neto_sc').AsFloat );
      if QSalidas.FieldByName('palets_tran_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('palets_tran_s').AsFloat * rGastosTPalet ) / QSalidas.FieldByName('palets_tran_sc').AsFloat );
      if QSalidas.FieldByName('kilos_tran_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('kilos_tran_s').AsFloat * rGastosTKilo ) / QSalidas.FieldByName('kilos_tran_sc').AsFloat );
      if QSalidas.FieldByName('cajas_tran_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('cajas_tran_s').AsFloat * rGastosTCaja ) / QSalidas.FieldByName('cajas_tran_sc').AsFloat );
      if QSalidas.FieldByName('neto_tran_sc').AsFloat <> 0 then
        rGastos:= rGastos + ( ( QSeccionesFactura.FieldByName('neto_tran_s').AsFloat * rGastosTImporte ) / QSalidas.FieldByName('neto_tran_sc').AsFloat );
      if rGastos > 0 then
      begin
...
procedure TDMDContabilizacion.SeccionesUnicaAbono;
var
  sSeccion: string;
begin
  with QSeccionContable do
  begin
    ParamByName('empresa').AsString:= QEmpresaProcedencia.FieldByName('emp_procedencia_sl').AsString;
    ParamByName('centro').AsString:= QEmpresaProcedencia.FieldByName('centro_origen_sl').AsString;
    ParamByName('producto').AsString:= QLineasAbono.FieldByName('producto_fal').AsString;
    Open;
    sSeccion:= FieldByName('sec_contable_sc').AsString;
    Close;
  end;
  PutSeccion( sSeccion, '', QLineasAbono.FieldByName('importe_fal').AsFloat, 0, 0 );
end;

procedure TDMDContabilizacion.SeccionesDistribuirAbono( const ALineas: Integer; const AKilos: Real );
var
  i: integer;
  rImporte, rAcum: Real;
  sSeccion: String;
begin
  QEmpresaProcedencia.First;
  rAcum:= 0;
  for i:= 1 to ALineas do
  begin
    ...
    rImporte:= SimpleRoundTo( ( QEmpresaProcedencia.FieldByName('kilos_sl').AsFloat * QLineasAbono.FieldByName('importe_fal').AsFloat ) / AKilos, -2 );
    rAcum:= rAcum + rImporte;

    PutSeccion( sSeccion, '', rImporte, 0, 0 );
    QEmpresaProcedencia.Next;
  end;
  ...
  rImporte:= QLineasAbono.FieldByName('importe_fal').AsFloat - rAcum;
  PutSeccion( sSeccion, '', rImporte, 0, 0 );
end;

procedure TDMDContabilizacion.SeccionesSalidasAbono;
var
  iLineas: integer;
  rKilosTotal: real;
begin
  with QEmpresaProcedencia do
  begin
    ParamByName('empresa').AsString:= QLineasAbono.FieldByName('empresa_fal').AsString;
    ParamByName('albaran').AsInteger:= QLineasAbono.FieldByName('n_albaran_fal').AsInteger;
    ParamByName('fecha').AsDateTime:= QLineasAbono.FieldByName('fecha_albaran_fal').AsDateTime;
    ParamByName('producto').AsString:= QLineasAbono.FieldByName('producto_fal').AsString;
    ParamByName('categoria').AsString:= QLineasAbono.FieldByName('categoria_fal').AsString;
    Open;
    ...
    while Not Eof do
    begin
      rKilosTotal:= rKilosTotal + FieldByName('kilos_sl').AsFloat ;
      Inc( iLineas );
      Next;
    end;

    case iLineas of
      0: PutSeccion( '', '', QLineasAbono.FieldByName('importe_fal').AsFloat, 0, 0 );
      1: SeccionesUnicaAbono;
      else SeccionesDistribuirAbono( iLineas, rKilosTotal );
    end;
    Close;
  end;
end;

function TDMDContabilizacion.AnyadirSeccionesAbono: Boolean;
var
  iLinea: integer;
  rAcumImporte: Real;
begin
  ...
  begin
    rAcumImporte:= rAcumImporte + aSecciones[iLinea].importe;
    aSecciones[iLinea].importeNeto:= aSecciones[iLinea].importe;
...
function TDMDContabilizacion.AnyadirSeccionesFactura: Boolean;
var
  iLinea: integer;
...
begin
  ...
  begin
    ...
    try
      ...
      InsertarLineaTFactura( AFileName );

      if not AnyadirSeccionesFactura then
      begin
        //raise Exception.Create('Factura descuadrada.');
      end;

      InsertarLineaEFactura;
    finally
      QSalidas.Close;
    end;
  end;
...
procedure TDMDContabilizacion.ContabilizarAbonoAutomatico( const AFileName: string );
begin
...
procedure TDMDContabilizacion.ContabilizarAbonoManual( const AFileName: string );
begin
  with QDocs do
  begin
    QLineasAbono.ParamByName('empresa').AsString:= QDocs.FieldByName('empresa_f').AsString;
    QLineasAbono.ParamByName('factura').AsInteger:= QDocs.FieldByName('n_factura_f').AsInteger;
    QLineasAbono.ParamByName('fecha').AsDate:= QDocs.FieldByName('fecha_factura_f').AsDateTime;
    QLineasAbono.Open;
    ...
    try
      ...
      InsertarLineaTAbono( AFileName );

      if not AnyadirSeccionesAbono then
      begin
        //raise Exception.Create('Factura descuadrada.');
      end;
...
begin
  if QDocs.FieldByName('concepto_f').AsString = 'M' then
  begin
    ContabilizarAbonoManual( AFileName );
  end
  else
  begin
    ContabilizarAbonoAutomatico( AFileName );
  end;
end;
...
procedure TDMDContabilizacion.InsertarLineaTFactura( const AFileName: string );
begin
  with facturas_conta_t do
  begin
    if not Active then
...
procedure TDMDContabilizacion.InsertarLineaTAbono( const AFileName: string );
var
  sDirSum, sTipoIva, sFacturaAsociada: string;
begin
  with QTipoIva do
  begin
    ParamByName('empresa').AsString:= QLineasAbono.FieldByName('empresa_fal').AsString;
    ParamByName('centro').AsString:= QLineasAbono.FieldByName('centro_salida_fal').AsString;
    ParamByName('albaran').AsInteger:= QLineasAbono.FieldByName('n_albaran_fal').AsInteger;
    ParamByName('fecha').AsDateTime:= QLineasAbono.FieldByName('fecha_albaran_fal').AsDateTime;
    Open;
    sTipoIva:= FieldByName('tipo_iva_sl').AsString;
    Close;
  end;
  ...
  begin
    ...
    ParamByName('fecha').AsDateTime:= QLineasAbono.FieldByName('fecha_albaran_fal').AsDateTime;
    Open;
    sDirSum:= FieldByName('dir_sum_sc').AsString;
    if FieldByName('fecha_factura_sc').AsDateTime < StrToDate( '1/7/2006') then
    begin
      sFacturaAsociada:= '';
    end
    else
    begin
      sFacturaAsociada:= NewCodigoFactura( QLineasAbono.FieldByName('empresa_fal').AsString,
                                 '380', sTipoIva,
  ...
  begin
    ...
    FieldByName('numero_fct').AsString:= sFactura;
    FieldByName('tipo_factura_fct').AsString:= 'AGC';
    FieldByName('planta_fct').AsString:= Planta;
    FieldByName('direccion_fct').AsString:= Direccion;
    FieldByName('fecha_conta_fct').AsDateTime:= QDocs.FieldByName('fecha_factura_f').AsDateTime;;
    ...
    FieldByName('fecha_albaran_fct').AsDateTime:= QLineasAbono.FieldByName('fecha_albaran_fal').AsDateTime;
    FieldByName('albaran_fct').AsString:= sDirSum + '-' +QLineasAbono.FieldByName('n_albaran_fal').AsString;
    FieldByName('factura_abono_fct').AsString:= FacturaAsociada;
    ...
    FieldByName('factura_abono_fct').AsString:= sFacturaAsociada;
...
begin
  ...
  begin
    ...
    FieldByName('linea_fcd').AsInteger:= ALinea + 1;
    FieldByName('cuenta_general_fcd').AsString:= sCtaPGC;
    FieldByName('cuenta_analitica_fcd').AsString:= sCtaPGA;
    FieldByName('importe_ai_fcd').AsFloat:= ABS( aSecciones[ALinea].importeNeto );
...
begin
  ...
  begin
    ...
    FieldByName('linea_fca').AsInteger:= ALinea + 1;
    ...
    FieldByName('importe_fca').AsFloat:= ABS( aSecciones[ALinea].importeNeto );
...
procedure TDMDContabilizacion.InsertarLineaEFactura;
begin
  with facturas_conta_e do
  begin
    if not Active then
    ...
    FieldByName('vencimiento_fce').AsDateTime:= FechaVencimiento( QSalidas.FieldByName('fecha_sc').AsDateTime );
    FieldByName('impuesto3_fce').AsString:= sTipoIva;
    FieldByName('modo_pago_fce').AsString:= sFormaPago;
    FieldByName('importe_divisa_fce').AsFloat:= ABS( QDocs.FieldByName('importe_total_f').AsFloat );
...
procedure TDMDContabilizacion.InsertarLineaEAbono;
begin
  with facturas_conta_e do
  begin
    if not Active then
    ...
    FieldByName('numero_fce').AsString:= sFactura;
    FieldByName('linea_fce').AsInteger:= 1;
    FieldByName('vencimiento_fce').AsDateTime:= FechaVencimiento( QLineasAbono.FieldByName('fecha_albaran_fal').AsDateTime );
    FieldByName('impuesto3_fce').AsString:= sTipoIva;
    FieldByName('modo_pago_fce').AsString:= sFormaPago;
    FieldByName('importe_divisa_fce').AsFloat:= ABS( QDocs.FieldByName('importe_total_f').AsFloat );
    FieldByName('direccion_fce').AsString:= '001';
...
begin
  ...
  begin
    ...
    sCad:= sCad + ';' + aux_facturas_conta_e.FieldByName('impuesto3_fce').AsString;    
